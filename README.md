...
Since v1.3.0 the condition-photos cell also carries a **Send pickup/return photo link** button, next to the completeness bar and again in each rental's *Condition photos* section. A row reading `0 of 1` is a row the owner wants to act on, and reaching the day board on his phone to act on it was the step in between. It posts to the condition-photos plugin's own `pn_vc_nudge` endpoint — see *Security model* — and shows what the last attempt actually did, read back from stored meta: `✓ link sent <when>`, or red with the reason it did not go.

On the list, the button targets the rental **still in play** — the open phase with the latest pickup date — so a returning customer's history does not make it ambiguous which rental is being chased. On a rental card it targets that rental, and only on the phase that is actually next.
...
- **The one state-changing control is still not a write from here.** The v1.3.0 *Send photo link* button is a form POST to **vehicle-condition's** `pn_vc_nudge` endpoint, which owns the token, the mail and the `_pn_vc_nudged_*` keys it stamps. Two narrow contracts cross the plugin boundary — the endpoint's action + nonce name, and the outcome vocabulary those keys store — and neither is a function call, so the no-runtime-dependency rule holds: the button hides itself (`has_action()`) when that plugin is inactive. The phase is re-derived by the endpoint from the locks and ignores anything the form says, so a stale page can mislabel a button but can never file return photos over a pickup set. The `back` URL that returns the owner to the row he tapped is rebuilt from sanitized params and validated same-host on arrival — an open redirect on a `nopriv` endpoint is not on offer.
