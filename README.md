...
- **Only the login-gated agreement link is rendered.** WP E-Signature also exposes a public `document_uri` that needs no login; that link is a bearer token and printing it into an admin page would put it in browser history and every screenshot. The admin reading this screen is already logged in, so the PDF route is the correct one.
